...
import static uk.q3c.util.text.MessageFormatMode.*
...
class DefaultMessageFormatTest extends Specification {
    ...
    def "format is valid"() {
        ...
        when:
        String result = new DefaultMessageFormat().format(mode, pattern, arguments)
    ...
    def "pattern with multiple parameters of same index"() {
        ...
        when: "strict"
        String result = new DefaultMessageFormat().format(pattern, arguments)
        ...
        result = new DefaultMessageFormat().format(STRICT_EXCEPTION, pattern, arguments)

        then: "throw exception"
        thrown MessageFormatException

        when: "lenient"
        result = new DefaultMessageFormat().format(LENIENT, pattern, arguments)
    ...
    def "parameter sequence missing an index,  number of args appears correct"() {
        ...
        when:
        new DefaultMessageFormat().format(STRICT_EXCEPTION, pattern, arguments)
