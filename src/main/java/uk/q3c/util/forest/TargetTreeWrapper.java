...
/**
 * Implementations of this interface wrap a tree of a particular type (for example a Vaadin Tree) to provide a
...
public interface TargetTreeWrapper<S, T> {
     ...
     *
     * @param parentNode      the parent target node which will be the parent of the created target node. This can be null, if the
     *                        new target node is to be a root.
     * @param sourceChildNode the source node on which the new target node is to be based. Cannot be null.
